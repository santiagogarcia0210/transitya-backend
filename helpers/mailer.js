...
console.log('[MAILER] config', {
  ...
  from:      process.env.FEROZO_FROM || 'info@transitya.com',
});

const FROM_EMAIL = process.env.FEROZO_FROM || 'info@transitya.com';
const ADMIN      = process.env.SUPERADMIN_EMAIL || 'info@transitya.com';

function brevoSend({ template, to, subject, htmlContent }) {
  return new Promise((resolve, reject) => {
    const body = JSON.stringify({
      sender:      { name: 'Transit·Ya', email: FROM_EMAIL },
      to:          [{ email: to }],
      subject,
      htmlContent,
    });
    ...
    const req = https.request(options, (r) => {
      ...
      r.on('end', () => {
        if (r.statusCode >= 200 && r.statusCode < 300) {
          ...
          console.log('[MAILER] enviado', { template, to, messageId: parsed.messageId });
          resolve(parsed);
        } else {
          console.error('[MAILER] fallo', { template, to, status: r.statusCode, body: data });
          reject(new Error(`Brevo ${r.statusCode}: ${data}`));
        }
      });
    });
    req.on('error', (err) => {
      console.error('[MAILER] fallo', { template, to, status: null, body: err.message });
      reject(err);
    });
    req.write(body);
    req.end();
