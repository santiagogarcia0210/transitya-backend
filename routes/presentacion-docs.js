...
const errHandler = (res, req, e) => {
  console.error('[PRESENTACION-DOCS]', req.path, e.message);
...
router.get('/:id', verifyToken, requireAdmin, async (req, res) => {
  try {
    const doc = await col(req.tenantId, 'presentacion_docs').doc(req.params.id).get();
...
router.put('/:id', verifyToken, requireAdmin, async (req, res) => {
  try {
    const ref = col(req.tenantId, 'presentacion_docs').doc(req.params.id);
