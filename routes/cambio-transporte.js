...
const errHandler = (res, req, e) => {
  console.error('[CAMBIO-TRANSPORTE]', req.path, e.message);
...
router.get('/:id', verifyToken, requireAdmin, async (req, res) => {
  try {
    const doc = await col(req.tenantId, 'cambio_transporte').doc(req.params.id).get();
...
router.put('/:id', verifyToken, requireAdmin, async (req, res) => {
  try {
    const ref = col(req.tenantId, 'cambio_transporte').doc(req.params.id);
