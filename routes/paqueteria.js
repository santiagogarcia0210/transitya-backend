...
const errHandler = (res, req, e) => {
  console.error('[PAQUETERIA]', req.path, e.message);
...
router.post('/repartidores', verifyToken, async (req, res) => {
  try {
    const data = req.body;
...
router.put('/repartidores/:id', verifyToken, async (req, res) => {
  try {
...
router.delete('/repartidores/:id', verifyToken, async (req, res) => {
  try {
    const ref = col(req.tenantId, 'paq_repartidores').doc(req.params.id);
    if (!(await ref.get()).exists) return res.status(404).json({ ok: false, mensaje: 'No encontrado.' });
...
router.get('/envios', verifyToken, async (req, res) => {
  try {
    const { estado, repartidorId, clienteId, texto, porPagina, pagina } = req.query;
    ...
    todos = todos.filter(doc => {
      ...
      if (estado       && doc.estado !== estado) return false;
      if (repartidorId && doc.repartidorId !== repartidorId) return false;
      if (clienteId    && doc.clienteId !== clienteId) return false;
      if (texto) {
        const q = normalizarText_(texto);
...
router.get('/envios-repartidor', verifyToken, async (req, res) => {
  try {
...
router.put('/envios/:id/estado', verifyToken, async (req, res) => {
  try {
...
// PATCH alias
router.patch('/envios/:id/estado', verifyToken, async (req, res) => {
  try {
    const { nuevoEstado, nota, fotoUrl } = req.body;
    const ESTADOS_VALIDOS = ['pendiente', 'asignado', 'en_camino', 'entregado', 'devuelto', 'cancelado'];
    if (!ESTADOS_VALIDOS.includes(nuevoEstado)) return res.status(400).json({ ok: false, mensaje: 'Estado inválido.' });
...
router.post('/envios/:id/asignar', verifyToken, requireAdmin, async (req, res) => {
  try {
    ...
    res.json({ ok: true, repartidorNombre: rep.nombre });
  } catch (e) { errHandler(res, req, e); }
});

router.post('/envios/:id/entregar', verifyToken, async (req, res) => {
  try {
...
router.get('/envios/:id', verifyToken, async (req, res) => {
  try {
...
router.post('/envios', verifyToken, async (req, res) => {
  try {
    const data = req.body;
    const id = data.id || randomUUID();
    ...
    res.json({ ok: true, id, nroEnvio });
  } catch (e) { errHandler(res, req, e); }
});

router.put('/envios/:id', verifyToken, async (req, res) => {
  try {
    const ref = col(req.tenantId, 'paq_envios').doc(req.params.id);
    if (!(await ref.get()).exists) return res.status(404).json({ ok: false, mensaje: 'Envío no encontrado.' });
    await ref.update({ ...req.body, actualizadoEn: new Date().toISOString() });
    res.json({ ok: true });
  } catch (e) { errHandler(res, req, e); }
});

router.delete('/envios/:id', verifyToken, requireAdmin, async (req, res) => {
  try {
    const ref = col(req.tenantId, 'paq_envios').doc(req.params.id);
    if (!(await ref.get()).exists) return res.status(404).json({ ok: false, mensaje: 'Envío no encontrado.' });
    await ref.delete();
...
router.get('/paquetes', verifyToken, async (req, res) => {
  try {
    const { estado, envioId, porPagina, pagina } = req.query;
    const snap = await col(req.tenantId, 'paq_paquetes').get();
    let todos = snap.docs.map(d => ({ ...d.data(), _fsId: d.id }));
    if (estado)  todos = todos.filter(d => d.estado === estado);
    if (envioId) todos = todos.filter(d => d.envioId === envioId);
    const pp = Number(porPagina) || 50;
    const pg = Math.max(1, Number(pagina) || 1);
...
router.post('/rutas', verifyToken, async (req, res) => {
  try {
    const data = req.body;
    const id = data.id || randomUUID();
    const doc = {
...
router.put('/rutas/:id', verifyToken, async (req, res) => {
  try {
    const ref = col(req.tenantId, 'paq_rutas').doc(req.params.id);
    if (!(await ref.get()).exists) return res.status(404).json({ ok: false, mensaje: 'No encontrado.' });
    await ref.update({ ...req.body, actualizadoEn: new Date().toISOString() });
    res.json({ ok: true });
  } catch (e) { errHandler(res, req, e); }
...
router.delete('/rutas/:id', verifyToken, requireAdmin, async (req, res) => {
  try {
    const ref = col(req.tenantId, 'paq_rutas').doc(req.params.id);
    if (!(await ref.get()).exists) return res.status(404).json({ ok: false, mensaje: 'No encontrado.' });
    await ref.delete();
